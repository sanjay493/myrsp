...
const yrlyProdnManager = () => {
  ...
  const fetchProdn = async () => {
    try {
      const response = await fetch("/api/yrlyProductionEntry");
  ...
  const addProdn = async () => {
    try {
      ...
      if (!newProdn.fy_year || !newProdn.unit || !newProdn.prodn) {
        console.error("All fields (fy_year, unit, prodn) are required.");
      ...
      if (result.success) {
        ...
        setNewProdn({ fy_year: "", unit: "", prodn: "" }); // Clear the form
      } else {
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-6 text-center">Production Records</h1>
      ...
      <div className="max-w-lg mx-auto mb-8 p-6 border rounded-lg shadow-lg bg-white">
        ...
        <div className="mb-4">
          <textarea
            placeholder="FY  (in 2024-25)"
            name="fy_year"
            value={newProdn.fy_year || ""}
            onChange={(e) => setNewProdn({ ...newProdn, [e.target.name]: e.target.value })}
            className="border p-2 rounded w-full"
          ></textarea>
        </div>
        ...
        <table className="table-auto w-full border-collapse border border-gray-400">
          <thead>
            <tr className="bg-gray-200">
              ...
              <th className="border border-gray-400 px-4 py-2">Financial Year</th>
          ...
          <tbody>
            {(() => {
              const groupedData = Prodn.reduce((acc, Prodn) => {
                if (!acc[Prodn.fy_year]) acc[Prodn.fy_year] = [];
                acc[Prodn.fy_year].push(Prodn);
                return acc;
              }, {});

              return Object.entries(groupedData).map(([fy_year, items]) => (
                items.map((item, index) => (
                  <tr key={item._id} className="hover:bg-gray-100">
                    {/* Merge cell for fy_year on the first occurrence */}
                    {index === 0 && (
                      <td
